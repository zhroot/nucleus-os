...
LOAD_ROOT:
     ...
     ; read root directory into memory
          mov     bx, Buffer                          ; copy root dir above bootcode
          call    ReadSectors
     ...
     ; read FAT into memory
          ...
          call    ReadSectors
     ; read image file into memory (0000:0500)
          mov     si, msgLdr
          call    DisplayMessage
...
;    DONE:
		  ...
		  movzx   edx,byte [DriveNumber]

		  jmp word 0x0000:0x0500
     FAILURE:
          mov     si, msgFailure
          call    DisplayMessage
          mov     ah, 0x00
...
.more
	 ...
	 ; es:bx is moved ahead to after the last byte read
     ;*************************************************************************
     ReadSectors:
     .MAIN
     ...
     .SECTORLOOP
          ...
          int     0x18                                ; that was very bad, reboot
     .SUCCESS
          mov     si, msgProgress
          call    DisplayMessage
          popa
...
.no_seg_carry
     ...
     cluster		dw 0x0000
     ImageName		db "NUCLOAD SYS"
     msgLoading		db "Loading...", 0x00
	 msgDir         db "Dir", 0x00
	 msgFat         db "FAT", 0x00
	 msgLdr         db "Loader", 0x00
     msgCRLF		db 0x0D, 0x0A, 0x00
     msgProgress	db ".", 0x00
	 msgBadProgress db "?", 0x00
     msgFailure		db 0x0D, 0x0A, "Loader not found!", 0x00
